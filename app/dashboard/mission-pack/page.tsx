  ...
  useEffect(() => {
    const fetchMissionPackLink = async () => {
      try {
        ...
        const { data: missionPackData, error: missionPackError } =
          await supabase
            .from("missionPacks")
            .select("link")
            .eq("stageId", stageId)
            .eq("category", "Junior-Scratch") // Added category filter
            .single();

        if (missionPackError || !missionPackData) {
          throw new Error(
            "Mission pack not found or not in Junior-Scratch category."
          );
        }
